...
# SD_QFormer
from imaginator.model.LLMSD_QFormer import BertModel
# LLaVA model
from imaginator.llava.model import LlavaLlamaForCausalLM
# CLIP text encoder
...
class LLMSD(LlamaPreTrainedModel):
    # _keys_to_ignore_on_load_missing = [r"lm_head.weight"]
    def __init__(self, config):
        super(LLMSD, self).__init__(config)
    ...
    # self.resize_token_embeddings(len(tokenizer)) -> update word-embedding
    ...
    # self.resize_token_embeddings(len(tokenizer)) -> update vec2word-head
    def set_output_embeddings(self, new_embeddings):
        self.lm_head = new_embeddings


    # setup_tokens -> conversation system num_new_tokens=33: "<img>"(system message) + " <img_0> ... <img_31>"
    def setup_tokens_for_conversation(
        ...
        # 32000 update to 32035 -> self.resize_token_embeddings(len(tokenizer))
        print(len(tokenizer))
        print(self.model.get_input_embeddings())
        print(self.lm_head)
        print('Original LLaMA word-embedding and language-model-head:', input_embeddings[0], output_embeddings[0])
    ...
    def init_visual_features_extractor(self, LLaVA_model_path, sd_qformer_version="v1.1-7b"):
        # Load LLaVA-LLaMA-v1.1 checkpoint... -> llava-1.1-7b vocab_size=32003
    ...
    def init_sd_qformer(
        ...
        # 1. use "bert-base-uncased" config.json -> add attributes -> encoder_width(LlamaModel)=4096, cross_attention_freq=2, query_length(SD q-former query)=77, num_hidden_layers=6

        # q-former model for LLM to SD
        self.sd_qformer = BertModel(config=qformer_config)
        # self-attention -> None，cross-attention keep the value
        ...
        # 2. SD_qformer_config in q-foremr BERTmodel: BertModel -> BertPreTrainedModel -> transformers/modeling_utils.py -> PreTrainedModel -> [1, query_length(SD q-former query)=77, SD_qformer_hidden_size=768]

    # 4.0 load pretrained checkpoint
    def load_pretrain_LLaVA_ckpt_v1_1(self, SD_QFormer_conversation_33tokens, LLaVA_00001, LLaVA_00002):
        """
        pretrain_model checkpoint dict
        # 1. 'model.embed_tokens.weight' -> word_embedding for '<img>' + '<img_0>...<img_31>' -> [num_new_tokens=33, LLM_hidden_size=4096]
        # 2. 'llm_proj.weight', 'llm_proj.bias' -> LLM_projection from ViT_qformer_hidden_size=768 to LLM_hidden_size=4096 -> [4096, 768], [4096]
        # 3. 'sd_query_tokens' -> [1, 77, 768], 'sd_qformer.embeddings', 'sd_qformer.encoder.layer'
        # 4. 'lm_head.weight' -> word_embedding linear decoder -> [num_new_tokens=33, LLM_hidden_size=4096]
        """

        # 0.1. SD_QFormer for conversation with 33 new_tokens checkpoint
        SD_QFormer_conversation_weights = torch.load(SD_QFormer_conversation_33tokens, map_location="cpu")
        # SD_QFormer_conversation_weights['model.embed_tokens.weight'].shape & SD_QFormer_conversation_weights['lm_head.weight'].shape -> [33, 4096]

        # 0.2. LLaVA_00001.bin for word-embeddings (32001='<im_start>' and 32002='<im_end>')
        LLaVA_00001_weights = torch.load(LLaVA_00001, map_location="cpu")

        # 0.3. LLaVA_00002.bin for language-model-head (32001='<im_start>' and 32002='<im_end>')
        LLaVA_00002_weights = torch.load(LLaVA_00002, map_location="cpu")

        # 1. word2vec: Embedding(32035, 4096) -> get SD_QFormer_conversation_33tokens first 1+ last 32 tokens，get LLaVA_00001 last 2 tokens
        SD_QFormer_conversation_weights_embed_tokens = SD_QFormer_conversation_weights.pop('model.embed_tokens.weight')[-self.config.sd_qformer_new_tokens:]
        self.model.embed_tokens.weight.data[-self.config.num_new_tokens:(-self.config.num_new_tokens + 1)] = \
            SD_QFormer_conversation_weights_embed_tokens[-self.config.sd_qformer_new_tokens:(-self.config.sd_qformer_new_tokens + 1)]
        self.model.embed_tokens.weight.data[(-self.config.num_new_tokens + 1):(-self.config.num_new_tokens + 3)] = \
            LLaVA_00001_weights['model.embed_tokens.weight'][-2:]
        self.model.embed_tokens.weight.data[(-self.config.num_new_tokens + 3):] = \
            SD_QFormer_conversation_weights_embed_tokens[(-self.config.sd_qformer_new_tokens + 1):]

        # 2. vec2word: Linear(in_features=4096, out_features=32035, bias=False) -> get SD_QFormer_conversation_33tokens first 1+ last 32 tokens，get LLaVA_00002 last 2 tokens
        SD_QFormer_conversation_weights_lm_head = SD_QFormer_conversation_weights.pop('lm_head.weight')[-self.config.sd_qformer_new_tokens:]
        self.lm_head.weight.data[-self.config.num_new_tokens:(-self.config.num_new_tokens + 1)] = \
            SD_QFormer_conversation_weights_lm_head[-self.config.sd_qformer_new_tokens:(-self.config.sd_qformer_new_tokens + 1)]
        self.lm_head.weight.data[(-self.config.num_new_tokens + 1):(-self.config.num_new_tokens + 3)] = \
            LLaVA_00002_weights['lm_head.weight'][-2:]
        self.lm_head.weight.data[(-self.config.num_new_tokens + 3):] = \
            SD_QFormer_conversation_weights_lm_head[(-self.config.sd_qformer_new_tokens + 1):]

        """
        LLaVA_00001_weights['model.embed_tokens.weight'][:-3] == self.origin_inp_embedding
        LLaVA_00002_weights['lm_head.weight'][:-3] == self.origin_out_embedding 
        """
        ...
        # 3. SD_QFormer query & 4. SD_QFormer
        self.sd_query_tokens.data = SD_QFormer_conversation_weights.pop('query_tokens').float()
        self.sd_qformer.load_state_dict({k[len('qformer.'):]: v for k, v in SD_QFormer_conversation_weights.items()})
        print('Loading SD_Qformer model checkpoint:', self.sd_qformer.load_state_dict({k[len('qformer.'):]: v for k, v in SD_QFormer_conversation_weights.items()}, strict=True))

        # 5. projection from LLaVA -> LLaVA_00002
        LLaVA_00002_projector = {'weight': LLaVA_00002_weights['model.mm_projector.weight'], 'bias': LLaVA_00002_weights['model.mm_projector.bias']}
        self.mm_projector.load_state_dict(LLaVA_00002_projector, strict=True)
        print('Loading LLaVA projector:', self.mm_projector.load_state_dict(LLaVA_00002_projector, strict=True))

    # 4.1. load pretrained checkpoint
    def load_pretrain_MLLM_alignment(self, SD_QFormer_conversation_33tokens, LLaVA_00002):
        """
        pretrain_model checkpoint dict
        # 1. 'mm_projector.weight', 'mm_projector.bias' -> LLM_projection from ViT_qformer_hidden_size=768 to LLM_hidden_size=4096 -> [4096, 768], [4096]
        # 2. 'sd_query_tokens' -> [1, 77, 768], 'sd_qformer.embeddings', 'sd_qformer.encoder.layer'
        # 3. 'lm_head.weight' -> word_embedding linear decoder -> [num_new_tokens=8, LLM_hidden_size=4096]
        # 4. 'model.embed_tokens.weight' -> word_embedding linear encoder -> [num_new_tokens=8, LLM_hidden_size=4096]
        """
    ...
    # 4.2. load pretrained unet checkpoint
    def load_pretrain_from_unet(self, unet_ckpt, is_convert=False, is_MagicBrush=False, is_InstructDiffusion=False):
        if is_convert == False:
            """ unet_ckpt """
            unet_weights = torch.load(unet_ckpt, map_location="cpu")
            unet_ckpt_new = {}
            for k, v in unet_weights.items():
                if 'unet.' in k:
                    unet_ckpt_new[k[len('unet.'):]] = v
            self.unet.load_state_dict(unet_ckpt_new, strict=True)
            print('Loading unet checkpoint:', self.unet.load_state_dict(unet_ckpt_new, strict=True))
        else:
            if is_MagicBrush == True:
                unet_path = unet_ckpt
                unet_weights = torch.load(unet_path, map_location="cpu")
                self.unet.load_state_dict(unet_weights, strict=True)
                print('Loading MagicBrush unet checkpoint:', self.unet.load_state_dict(unet_weights, strict=True))
            if is_InstructDiffusion == True:
                unet_path = unet_ckpt
                unet_weights = torch.load(unet_path, map_location="cpu")
                self.unet.load_state_dict(unet_weights, strict=True)
                print('Loading InstructDiffusion unet checkpoint:', self.unet.load_state_dict(unet_weights, strict=True))
    ...
    def load_pretrained_for_inference(self, pretrain_model, LLaVA_00002_weights):
        ...
        # 1. vec2word: Linear(in_features=4096, out_features=32008, bias=False) -> [32008, 4096](weight.data requires_grad=True) -> [8, 4096](get last 8 tokens)
        LLaMA_lm_haed = weights.pop('lm_head.weight')
        LLaMA_lm_haed = LLaMA_lm_haed[-self.config.num_new_tokens:]
        self.lm_head.weight.data[-self.config.num_new_tokens:] = LLaMA_lm_haed
        original_LLaMA_lm_head = self.original_lm_head_value
    ...
    def inference_llm(
        ...
        # type(self.model) -> <class 'imaginator.test.DS_PeftForLoRA.PeftModelForCausalLM'>
    ...
    def inference_sd_qformer(
        ...
        # for inference，hidden_states is '<img_0>...<img_7>' 8 LLM embedding -> [1, query_length(ViT q-former query)=8, LLM_hidden_size=4096]
    ...
    def forward(
        ...
        # 1. get Generated caption(input_ids) LLM embeddings
        batch_size = original_img.shape[0]
        IGNORE_TOKEN_ID = generated_caption_targets[0][0].item()

        # 2. use LLaVA to get image features
        ...
        # [bs, mm_projection_length=256, LLM_hidden_size=4096]
        ...
        # [bs, mm_projection_length=256], [bs, mm_projection_length=256](-100)
        ...
        # [bs, mm_projection_length=256](False=0)

        # for LLM new embeddings -> concat(ViT_qformer vision query + text prompt)-> LLM_img_start_token_id -> <img_0>=32001
        ...
        for sample in range(batch_size):
            ...
            # 4.1. input_ids -> [(editing_max_length-mm_projection_length)=256, LLM_hidden_size=4096]
            ...
            """ new_input_ids[:LLM_img_start_token_id_pos], new_input_ids[LLM_img_start_token_id_pos:] """
            ...
            # 4.2.1. new attention masks for LLM -> 1 for not padding
            ...
            # [Insert Subject Prompt Attention Mask, LLM Attention Mask] -> [1, editing_max_length=512]
            ...
            # [Insert Subject Prompt Target, LLM Target] -> [1, editing_max_length=512]
            ...
            # [Insert Subject SDLLM Attention Mask, SDLLM Attention Mask] -> [1, editing_max_length=512]
        ...
        # [bs, editing_max_length=512, LLM_hidden_size=4096], [bs, editing_max_length=512], [bs, editing_max_length=512], [bs, editing_max_length=512]
        ...
        # [bs, editing_max_length=512, LLM_hidden_size=4096], [bs, (editing_max_length-1)=511]

        # 5. Next token prediction language model loss: Enable model parallelism
        hidden_states = hidden_states.to(torch.float32)
        logits = self.lm_head(hidden_states)
        shift_logits = logits[..., :-1, :].contiguous()
        # [bs, editing_max_length=512, LLM_new_vocab_size=32035], [bs, (editing_max_length-1)=511, LLM_new_vocab_size=32035]
        ...
        # 6. SD-QFormer -> transform hidden state from LLM-space to CLIP-space, using BERT-like QFormer
        hidden_states = hidden_states[:, :self.editing_max_length, :]
        SDLLM_attention_masks = SDLLM_attention_masks[:, :self.editing_max_length]
        # [bs, editing_max_length=512, LLM_hidden_size=4096], [bs, editing_max_length=512]
        query_tokens = self.sd_query_tokens.expand(hidden_states.shape[0], -1, -1)
        query_output = self.sd_qformer(
            query_embeds=query_tokens,
            encoder_hidden_states=hidden_states,
        ...
        # [bs, query_length(SD q-former query)=CLIP_model_max_length=77, SD_qformer_hidden_size=CLIP_test_dim=768]
        ...
        # Final text conditioning -> if prompt_mask==True: null-text "" replace
        ...
        # CFG-2. Sample masks for the original images -> 5%-15% image condition=null
