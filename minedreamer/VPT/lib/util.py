...
import minedreamer.VPT.lib.torch_util as tu
from minedreamer.VPT.lib.masked_attention import MaskedAttention
from minedreamer.VPT.lib.minecraft_util import store_args
from minedreamer.VPT.lib.tree_util import tree_map

from minedreamer.helpers import print_recursive_shape
...
class ResidualRecurrentBlocks(nn.Module):
    ...
    def forward(self, x, first, state):
        ...
        debug = False
        if debug:
            print('\n*in ResidualRecurrentBlocks forward*')
            print_recursive_shape('x', x)
            print_recursive_shape('first', first)
            print_recursive_shape('state', state)
        
        for i, (block, _s_in) in enumerate(zip(self.blocks, state)):
            if debug:
                print(f'\n\t{i} block = {type(block)}')
                print_recursive_shape(f'\t{i} _s_in', _s_in)
            
            x, _s_o = block(x, first, _s_in)

            if debug:
                print_recursive_shape(f'\t{i} x', x)
                print_recursive_shape(f'\t{i} _s_o', _s_o)

            state_out.append(_s_o)
        
        if debug:
            print_recursive_shape('x', x)
            print_recursive_shape('state_out', state_out)
            print('\n*End of ResidualRecurrentBlocks forward*')
...
class ResidualRecurrentBlock(nn.Module):
    ...
    def __init__(
        ...
        elif recurrence_type == "transformer":  # BB4RL: we use transformer
