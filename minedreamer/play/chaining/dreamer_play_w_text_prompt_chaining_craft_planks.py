...
block_filter_types = ["_log", "_planks"]

block_names = ["log", "planks"]

def play_interactive(args, cvae_manager, metadata):

    prompt_id = 0
    prompt_list = ["chop a tree", "craft wooden planks"]
    ...
    prog_evaluator = ProgrammaticEvaluator(state['obs'])
    ...
    for i in tqdm(range(3000)):
        ...
        prog_evaluator.update(state['obs'])

        if i == 1500:
            print("Change prompt...")
            is_new_prompt = True

    video_name = f"craft_planks"
...
def main(args):
    ...
    for i in range(1, args.times + 1):
        args.save_video_dir = os.path.join(save_video_dir, f"craft_planks_{i}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_video_dir', type=str, default='data/play/chaining/craft_planks')
