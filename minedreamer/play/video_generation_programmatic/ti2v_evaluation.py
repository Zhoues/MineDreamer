...
def play(args, metadata):
    ...
    for i in tqdm(range(args.gameplay_length)):
        ...
        if i % args.freq == 0:
            ...
            video_save_path = video_dreamer.generate_ti2v_video(suffix="ti2v_generation", text_prompt=text_prompt, video_save_path=os.path.join(video_generation_save_dir, f"{i}.mp4"),
                                                                current_image_path=os.path.join(current_image_save_dir, f"{i}.png"), is_del=1)
...
def main(args):
    ...
    if isinstance(args.env_seed, int):
        ...
        for i in range(args.times):
            ...
            args.save_dirpath = f'data/play/video_generation_programmatic/ti2v/{model_name}_' + str(text_prompt.replace(' ', '_')) + '_cond_scale_' + str(args.cond_scale) + '_length_' + str(args.gameplay_length) + '_seed_' + str(args.env_seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, help='T+I2V model name for evaluation.')
