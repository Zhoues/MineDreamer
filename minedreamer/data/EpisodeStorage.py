...
class EpisodeStorage:
    ...
    def update_episode_dirpath(self, episode_dirpath):
        ...
        self.frames_dirpath = os.path.join(self.episode_dirpath, 'frames.mp4')  # fps=20 image_shape=(128, 128)
        ...
        self.one_frame_embeds_attn_filepath = os.path.join(self.episode_dirpath, 'one_frame_embeds_attn.pkl')
    ...
    def append(self, agent_input_pov: np.ndarray, action: dict, video_embed_attn: Optional[np.ndarray], one_frame_embed_attn: Optional[np.ndarray]):
        ...
        self.one_frame_embeds_attn.append(one_frame_embed_attn)
    
    def load_frames(self, only_range=None, to_rgb=True):
        """Loads the frames by reading from the MP4 file."""
        frames = minedreamer.utils.video_utils.load_video_to_lst(self.frames_dirpath, to_rgb=to_rgb,
    ...
    def load_one_frame_embeds_attn(self) -> list[np.ndarray]:
        """Note: first 16 are always None."""
        with open(self.one_frame_embeds_attn_filepath, 'rb') as f:
            one_frame_embeds_attn = pickle.load(f)
        # Fix since some are saved as lists in the wrong dtype
        for i in range(len(one_frame_embeds_attn)):
            if one_frame_embeds_attn[i] is not None and not isinstance(one_frame_embeds_attn[i], np.ndarray):
                one_frame_embeds_attn[i] = np.array(one_frame_embeds_attn[i], dtype=np.float32)
                one_frame_embeds_attn[i] = one_frame_embeds_attn[i].reshape(1, -1)
        return one_frame_embeds_attn
    ...
    def save_one_frame_embeds_attn(self, one_frame_embeds_attn: list[np.ndarray]):
        with open(self.one_frame_embeds_attn_filepath, 'wb') as f:
            pickle.dump(one_frame_embeds_attn, f)
    ...
    def is_valid(self, for_training: bool = False, min_frames_training: int = 720,
        ...
        if not os.path.exists(self.one_frame_embeds_attn_filepath):
            return False, 'one_frame_embeds_attn_filepath does not exist'
    ...
    def save_episode(self):
        ...
        # Save the one_frame_embeds_attn
        with open(self.one_frame_embeds_attn_filepath, 'wb') as f:
            pickle.dump(self.one_frame_embeds_attn, f)
